		...
		swal({
	        ...
	        if (value.value) {
	        	$.ajax({
	    			...
	    			data: formData,
	                success: function() {
	                    	window.location.href = "/CFA101G4/back-end/Orders/selectOD.jsp"
	                }
	            })
	        } else {
	            swal('已取消操作！', '', 'error')
...
function frist(start,rows){
 			$.ajax({
 				...
 				success:function(data){
 					...
 					if(data[i].od_sta == 7 && data[i].od_goldflow == 1 ){
 						button = `<button type="button" id="submit" class="btn btn-primary m-0" value="${data[i].od_id}">撥款</button>
 						`;
 						checkbox=`<input class="submitcheck" type="checkbox" name="submitodid" value="${data[i].od_id}" >`
 						sta=`<button type="button" class="btn btn-secondary" disabled>待撥款</button>`;
 					}else if(data[i].od_sta == 8 && data[i].od_goldflow == 2){
 						button = `<button type="button" id="refund" class="btn btn-danger m-0" value="${data[i].od_id}">退款</button>
 						`;
 						checkbox=`<input  class="refundcheck" type="checkbox" name="refundodid" value="${data[i].od_id}" >`;
 						sta=`<button type="button" class="btn btn-secondary" disabled>待退款</button>`;
 					}else if(data[i].od_sta == 8 && data[i].od_goldflow == 4){
 						button = `<button type="button" id="refund" class="btn btn-danger m-0" value="${data[i].od_id}" disabled>已退款</button>
 	 						`;
 						sta=`<button type="button" class="btn btn-success" disabled>已退款</button>`;
 					}else{
 						button = `<button type="button" id="refund" class="btn btn-primary m-0" value="${data[i].od_id}" disabled>已撥款</button>
 						`;
 					...
 					<div class="listbody">
 						<div class="row justify-content-center border-bottom border-$gray-500 p-3">
				            <div class="col-1 text-center align-self-center">`+
				            	checkbox
				              +`</div>
				              <div class="col-1 text-center align-self-center">${data[i].od_id}</div>
				              <div class="col-1 text-center align-self-center">${data[i].od_price}</div>
				              <div class="col-2 text-center align-self-center">${data[i].od_date}</div>
				              <div class="col-1 text-center align-self-center">${data[i].od_payment}</div>
				              <div class="col-2 text-center align-self-center" id="odsta${data[i].od_id}">`+sta+`</div>
				              <div class="col-1 text-center align-self-center">${data[i].od_point}</div>
				              <div class="col-3 text-center align-self-center">`+
				              	button
				               +`<button type="button" id="showOD" value="${data[i].od_id}" class="btn btn-secondary m-0">查看訂單</button>
				              </div>
				        </div>
 					</div>
		 			          `
				...
				sta=``;
					...
					if(data[i].od_sta == 7 && data[i].od_goldflow == 1){
						...
						button = `<button type="button" id="refund" class="btn btn-danger m-0" value="${data[i].od_id}" disabled>已退款</button>
	 						`;
						sta=`<button type="button" class="btn btn-success" disabled>已退款</button>`;
					}else if(data[i].od_sta == 7 && data[i].od_goldflow == 3){
						button = `<button type="button" id="refund" class="btn btn-primary m-0" value="${data[i].od_id}" disabled>已撥款</button>
						`;
						sta=`<button type="button" class="btn btn-success" disabled>已撥款</button>`;
					}else{
