...
<!--           購物車圖示結尾  -->
 ...
 <nav class="container-fluid">
        <div class="row mt-3" id =i1>
<!--             左邊的分類欄位 -->
<!--             <div class="col-2" style="margin-left:50px ;" > -->
<!--                 <div class="col" style="margin-top: 5px;"> -->
<!--                     分類 -->
<!--                     <div class="form-check" style="margin: 5px "> -->
<!--                         <input class="form-check-input" type="checkbox" value="" id="flexCheckDefault" style="margin: 0px; margin-right: 5px;"> -->
...
<!--                             篩選 -->
<!--                             <button type="button" class="btn btn-outline-secondary">最新上架</button> -->
<!--                             <button type="button" class="btn btn-outline-secondary">熱門產品</button> -->
...
<!-- 	每個商品起始頭 -->
        <c:forEach var="pd" items="${spb.list }">
...
<nav aria-label="Page navigation example" style="margin : 0px auto">
  ...
  <c:if test="${spb.currentPage == 1 }">
  	<li class="page-item disabled">
      <a class="page-link" href="<%=request.getContextPath()%>/ProductServlet?action=search&scurrentPage=${spb.currentPage -1}&srows=20&pname=${pname}&pcgid=${pcgid}" aria-label="Previous">
  ...
  <c:if test="${spb.totalPage <= 10 }">
	    <c:forEach begin="1" end="${spb.totalPage}" var="spg">
	    	 <c:if test="${spb.currentPage == spg}">
	    	 	  <li class="page-item active"><a class="page-link" href="<%=request.getContextPath()%>/ProductServlet?action=search&scurrentPage=${spg}&srows=20&pname=${pname}&pcgid=${pcgid}">${spg}</a></li>
	    	 </c:if>
	    	 <c:if test="${spb.currentPage != spg}">
	    	 	  <li class="page-item"><a class="page-link" href="<%=request.getContextPath()%>/ProductServlet?action=search&scurrentPage=${spg}&srows=20&pname=${pname}&pcgid=${pcgid}">${spg}</a></li>
	    	 </c:if>
	    </c:forEach>
    </c:if>
    ...
    <c:if test="${spb.currentPage > 5 && spb.totalPage > 10 && (spb.totalPage - spb.currentPage) >= 5 }">
	    <c:forEach begin="${spb.currentPage - 5}" end="${spb.currentPage + 5}" var="spg">
	    	 <c:if test="${spb.currentPage == spg}">
	    	 	  <li class="page-item active"><a class="page-link" href="<%=request.getContextPath()%>/ProductServlet?action=search&scurrentPage=${spg}&srows=20&pname=${pname}&pcgid=${pcgid}">${spg}</a></li>
	    	 </c:if>
	    	 <c:if test="${spb.currentPage != spg}">
  	...
  	<c:if test="${spb.currentPage <= 5 && spb.totalPage >= 10 }">
	    <c:forEach begin="1" end="10" var="spg">
	    	 <c:if test="${spb.currentPage == spg}">
	    	 	  <li class="page-item active"><a class="page-link" href="<%=request.getContextPath()%>/ProductServlet?action=search&scurrentPage=${spg}&srows=20&pname=${pname}&pcgid=${pcgid}">${spg}</a></li>
	    	 </c:if>
	    	 <c:if test="${spb.currentPage != spg}">
    ...
    <c:if test="${ (spb.totalPage - spb.currentPage) < 5 && spb.totalPage >10}">
    <c:forEach begin="${spb.currentPage - 5}" end="${spb.totalPage}" var="spg">
    	 <c:if test="${spb.currentPage == spg}">
    	 	  <li class="page-item active"><a class="page-link" href="<%=request.getContextPath()%>/ProductServlet?action=search&scurrentPage=${spg}&srows=20">${spg}</a></li>
    	 </c:if>
    	 <c:if test="${spb.currentPage != spg}">
    	 	  <li class="page-item"><a class="page-link" href="<%=request.getContextPath()%>/ProductServlet?action=search&scurrentPage=${spg}&srows=20">${spg}</a></li>
    	 </c:if>
    </c:forEach>
    </c:if>
  	...
  	<c:if test="${spb.currentPage > 5 && (spb.totalPage - spb.currentPage) >= 5 } ">
    <c:forEach begin="${spb.currentPage-5}" end="${spb.currentPage+5}" var="spg">
    	 <c:if test="${spb.currentPage == spg}">
    	 	  <li class="page-item active"><a class="page-link" href="<%=request.getContextPath()%>/ProductServlet?action=search&scurrentPage=${spg}&srows=20&pname=${pname}&pcgid=${pcgid}">${spg}</a></li>
    	 </c:if>
	...
	<c:if test="${spb.currentPage == spb.totalPage }">
    <li class="page-item disabled">
      <a class="page-link" href="<%=request.getContextPath()%>/ProductServlet?action=search&scurrentPage=${spb.currentPage +1 }&srows=20&pname=${pname}&pcgid=${pcgid}" aria-label="Next">
  ...
  總頁數${spb.totalPage }總筆數${spb.totalCount }
</div>
<!-- 分頁 -->
