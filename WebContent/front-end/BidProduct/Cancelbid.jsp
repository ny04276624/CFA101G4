        ...
        <div class="container-fluid">
            ...
            <div class="row">
                ...
                <div class="col">
                    ...
                    <!-- 訂單的開頭預設結束點 -->
                    <!-- 一張訂單的起始點 -->
                    <c:if test="${bid.cancelbids(memberVO.mem_id).size() == 0}">
                    <div class="col-12 text-center align-self-center">
                          	  目前無待出貨商品
                    </div>
                    </c:if>
                    <c:forEach var="can" items="${bid.cancelbids(memberVO.mem_id)}">
                    
                    <div class="row justify-content-center p-3">
                        <div class="col-2 text-center align-self-center">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="bp_id" value="${can.bp_id }" id="bidcheckbox">
                        ...
                        <div class="col-2 text-center align-self-center">
                            <fmt:formatDate value="${can.bp_etime}" pattern="yyyy-MM-dd HH:mm:ss"/>
                        </div>
                        <div class="col-1 text-center align-self-center">
                            ${can.bp_to }
                        </div>
                        <div class="col-1 text-center align-self-center">         	
								<c:if test="${can.bp_shsta == 5}">買家申請退貨</c:if>
								<c:if test="${can.bp_shsta == 6}">賣家接受退貨:等待撥款</c:if>
								<c:if test="${can.bp_shsta == 7}">賣家拒絕退貨</c:if>
                        </div>
                        <div class="col-3 text-center align-self-center">
                        	<c:if test="${can.bp_shsta == 5}"><button type="button" id="accept" value="${can.bp_id }" class="btn btn-primary m-0">接受退貨</button></c:if>
                        	<c:if test="${can.bp_shsta == 6 || can.bp_shsta == 7}"><button type="button" id="accept" value="${can.bp_id }" class="btn btn-primary m-0" disabled>接受退貨</button></c:if>
                        	<c:if test="${can.bp_shsta == 5}"><button type="button" id="cancel" value="${can.bp_id }" class="btn btn-danger m-0" >拒絕退貨</button></c:if>   
                        	<c:if test="${can.bp_shsta == 6 || can.bp_shsta == 7}"><button type="button" id="cancel" value="${can.bp_id }" class="btn btn-danger m-0" disabled>拒絕退貨</button></c:if>   
                        </div>
                        <div class="col-1 text-center align-self-center">
                        	<button type="button" class="btn btn-secondary" data-bs-toggle="modal" data-bs-target="#bmemid${can.bp_id }">聯絡買家</button>
                        </div>
                        <div class="col-1 text-center align-self-center">
                            <!-- Button trigger modal -->
                            <button type="button" class="btn btn-secondary" data-bs-toggle="modal"
                            data-bs-target="#bmemid${can.bp_id }">查看競標商品詳情</button>
                            <!-- Modal -->
                            <div class="modal fade" id="bmemid${can.bp_id }" data-bs-backdrop="static"
                                data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel"
                                aria-hidden="true">
                                <div class="modal-dialog modal-xl">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="staticBackdropLabel">買家 ${can.bp_bmemid } 的競標明細</h5>
                                        ...
                                        <div class="modal-body">
                                            <div class="container-fluid">
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-2 text-center align-self-center">
                                                        <c:if test="${bpi.check(can.bp_id) }">
                            								<img src="data:image/gif;base64, ${bpi.get(can.bp_id)}" class="img-thumbnail" alt="...">
                        								</c:if>
                        								<c:if test="${bpi.check(can.bp_id) == false}">
                                                    ...
                                                    <div class="col-2 text-center align-self-center">
														<c:if test="${can.bp_shsta == 2}"><td >訂單尚未出貨</td></c:if>
														<c:if test="${can.bp_shsta == 3}"><td>訂單已出貨</td></c:if>
														<c:if test="${can.bp_shsta == 4}"><td>訂單已完成</td></c:if>
														<c:if test="${can.bp_shsta == 5}"><td>買家申請退貨</td></c:if>
														<c:if test="${can.bp_shsta == 6}"><td>賣家接受退貨</td></c:if>
														<c:if test="${can.bp_shsta == 7}"><td>賣家拒絕退貨</td></c:if>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="row">
                                                	<div class="col align-self-center">
                                                       		 配送地址:
                                                       	<p class="h5">${can.bp_add}</p>
                                                    </div>
                                                    
                                                    <div class="col align-self-center">
                                                      	 	 收件人姓名:
                                                        <p class="h5">${can.bp_bname }</p>
                                                    </div>
                                                    <div class="col align-self-center">
                                                      	 	 連絡電話:
                                                        <p class="h5">${can.bp_tel }</p>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
...
<script type="text/javascript" src="/CFA101G4/JS/Front-end-js/bid_order/cancel.js"></script>
